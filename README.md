...
журнала. Доступен отбор журналов информационных баз к экспорту по регулярному выражению имени информационной базы
...
- Auth
  ...
  - TokensEndpoint - адрес конечной точки издателя токенов аутентификации (`client_credentials` grant, OAuth2). Это может быть:
    - внешний OIDC/SSO провайдер (пример выше) - в этом случае сервер oneswiss должен быть настроен в режиме `Auth:Mode=Mixed` (см. раздел про сервер) с `Auth:Jwt:SigningKey` не заданным (пустым) - иначе сервер не примет токены от внешнего провайдера;
    - собственный `oneswiss-server` - укажите `https://{адрес сервера}/api/auth/token` (используйте ту же схему - `http`/`https`, - по которой сервер реально доступен: если сервер поднят без TLS-терминации, например, напрямую по `Http:Port` без реверс-прокси, укажите `http://...`, иначе агент не сможет получить токен и будет молча подключаться без аутентификации, получая 401 при рукопожатии WebSocket). Работает при любом `Auth:Mode`, независимо от `Auth:Jwt:SigningKey` (использует тот же ключ, что и логин пользователей веб-панели). `ClientId`/`ClientSecret` в этом случае выпускаются на странице «Секреты агентов» веб-панели (раздел «Общее», только для администраторов) - секрет показывается один раз при создании и не восстанавливается.
