...
### Сервис получения списков общих информационных баз
Платформа 1С позволяет получать клиентам списки информационных баз с помощью веб-сервиса WebCommonInfoBases.  
В OneSwiss реализован интерфейс сервиса и прикладные объекты для обеспечения его работы в связке с контроллером домена.
Для начала использования сервиса на клиентских устройствах, в файле 1cestart.cfg необходимо прописать параметр с адресом конечной точки сервиса по шаблону:
```ini
InternetService={URL}/api
```
Пример:
```ini
InternetService=http://localhost:7002/api
```
Точка получения списка ИБ требует аутентификации пользователя на сервере OneSwiss, который использует в этих целях kerberos билеты.
