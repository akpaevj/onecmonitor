...
public static partial class V8Services
{
    ...
    {
        return GetRagentServices(platforms).Where(c => c.IsActive).ToList();
    }

    public static List<RagentService> GetRagentServices(IReadOnlyList<V8Platform> platforms)
    {
        return GetV8Services(platforms).Where(c => c is RagentService).Select(c => (c as RagentService)!).ToList();
    }

    private static List<RasService> GetActiveRasServices(IReadOnlyList<V8Platform> platforms)
    {
        return GetRasServices(platforms).Where(c => c.IsActive).ToList();
    }

    public static List<RasService> GetRasServices(IReadOnlyList<V8Platform> platforms)
    {
        return GetV8Services(platforms).Where(c => c is RasService).Select(c => (c as RasService)!).ToList();
    }
    ...
    {
        return GetCrServerServices(platforms).Where(c => c.IsActive).ToList();
    }
    ...
    private static List<V8Service> GetV8Services(IReadOnlyList<V8Platform> platforms)
    {
        return Environment.OSVersion.Platform == PlatformID.Win32NT
            ...
            : GetLinuxDaemons(platforms);
    }

    private static bool CheckExecContainsV8ServiceExecutable(string exec)
    {
        return exec.Contains("ras") || exec.Contains("ragent");
    }
...
#pragma warning disable CA1416
    ...
    {
        return GetServiceKeyValue(serviceName, "ImagePath");
    }

    private static string? GetObjectName(string serviceName)
    {
        return GetServiceKeyValue(serviceName, "ObjectName");
    }
...
#pragma warning restore CA1416
    ...
    private static List<V8Service> GetLinuxDaemons(IReadOnlyList<V8Platform> platforms)
    {
        ...
        while (true)
        {
            ...
            var description = RunCommandWithBash($"systemctl show {name} -p Description").Replace("Description=", "")
                .Trim();
        ...
        args.Items.ForEach(argsItem =>
        {
            switch (argsItem)
            {
                ...
                case ArgsParameter parameter when parameter.Value.StartsWith('$'):
                    parameter.Value = GetExecStartArgValue(serviceName, parameter.Value)?.TrimStart('-') ??
                                      string.Empty;
    ...
    {
        return RunCommandWithBash($"systemctl show {name} -p {variable}").Trim();
    }
