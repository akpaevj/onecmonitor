...
import {
  getClusterServerRules,
  getClusterServers,
  getServersAdministrationOverview,
  type V8AssignmentRuleItem,
  type V8ServerItem,
} from "@/lib/api/servers-administration";
import { cn } from "@/lib/utils";

type ClusterRulesPageProps = {
  params: Promise<{ id: string }>;
};

export default function ClusterRulesPage({ params }: ClusterRulesPageProps) {
  ...
  const [items, setItems] = useState<V8AssignmentRuleItem[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (serverId) {
        const data = await getClusterServerRules(clusterId, serverId);
        setItems(data);
      } else {
        setItems([]);
      }
      ...
      if (e instanceof ApiError && typeof e.details === "string") {
        ...
        setError("Не удалось загрузить правила назначения");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle>Правила назначения функциональности</CardTitle>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-left">
              <tr>
                ...
                <th className="px-3 py-2 font-medium">Тип объекта</th>
                <th className="px-3 py-2 font-medium">Информационная база</th>
                <th className="px-3 py-2 font-medium">Тип правила</th>
              ...
              {isLoading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="border-t">
                    ...
                    <td className="px-3 py-2">{item.objectType || "—"}</td>
                    <td className="px-3 py-2">{item.infoBaseName || "Все"}</td>
                    <td className="px-3 py-2">{item.ruleType}</td>
                    <td className="px-3 py-2">{item.applicationExt || "—"}</td>
