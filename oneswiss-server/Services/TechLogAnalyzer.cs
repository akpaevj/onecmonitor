...
public class TechLogAnalyzer(IServiceProvider serviceProvider, IMapper mapper, ILogger<TechLogAnalyzer> logger)
{
    ...
    private async Task CompleteWithNestedCalls(TjEvent tjEvent, List<CallGraphMember> chain,
    ...
    private async Task CompleteWithNestedScalls(TjEvent tjEvent, List<CallGraphMember> chain,
        CancellationToken cancellationToken)
    {
        await InitConnection(cancellationToken);
    ...
    public async Task<Dictionary<Guid, LockWaitingGraphMember>> GetLockWaitingGraph(Guid id,
        CancellationToken cancellationToken)
    {
        await InitConnection(cancellationToken);
        ...
        else
        {
            ...
            {
                await FillLockWaitingGraphVertices(tjEvent, LockWaitingTimelineMemberType.Victim, graph,
                    cancellationToken);
            }
            else if (tjEvent.EventName == "TTIMEOUT")
            {
                ...
                if (tlock == null)
                    ...
                    await FillLockWaitingGraphVertices(tlock, LockWaitingTimelineMemberType.Victim, graph,
                        cancellationToken);
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var culpritConnectionId in tlock.WaitConnections)
        {
            ...
            {
                await FillLockWaitingGraphVertices(culpritTlock,
                    GetMemberType(LockWaitingTimelineMemberType.DirectCulprit, memberType), graph, cancellationToken);
            ...
            else
            {
                ...
                {
                    await FillLockWaitingGraphVertices(culpritTlock,
                        GetMemberType(LockWaitingTimelineMemberType.DirectCulprit, memberType), graph,
                        cancellationToken);
                    vertex.DirectCulprits.Add(culpritTlock.Id);
                }
                else
        ...
        {
            await FillLockWaitingGraphVertices(indirectCulprit,
                GetMemberType(LockWaitingTimelineMemberType.IndirectCulprit, memberType), graph, cancellationToken);
            vertex.IndirectCulprits.Add(indirectCulprit.Id);
        }
    }

    private static LockWaitingTimelineMemberType GetMemberType(LockWaitingTimelineMemberType target,
        LockWaitingTimelineMemberType current)
    {
        return current == LockWaitingTimelineMemberType.IndirectCulprit
            ? LockWaitingTimelineMemberType.IndirectCulprit
            : target;
    }
    ...
    private async Task InitConnection(CancellationToken cancellationToken)
    {
        ...
        _repository = new ClickHouseContext(dtoSettings.Dbms, dtoSettings.Credentials, dtoSettings.DatabaseName,
            dtoSettings.Table);
