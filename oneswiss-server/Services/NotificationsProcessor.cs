...
public class NotificationsProcessor(
    ...
    ILogger<NotificationsProcessor> logger) : BackgroundService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (notification.Type == NotificationType.ErrorReportReceived)
            ...
            await botClient.SendMessage(chatId, notification.Message, ParseMode.MarkdownV2,
                cancellationToken: cancellationToken);
    }

    private static async Task SendWebHookNotification(Notification notification, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var item = context.ErrorReports.AsNoTracking()
            .FirstOrDefault(x => x.Id == Guid.Parse(notification.Additionalinfo));
