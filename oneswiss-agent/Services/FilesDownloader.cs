...
public class FilesDownloader(ILogger<FilesDownloader> logger) : IDisposable
{
    ...
    public async Task<Dictionary<Guid, string>> Download(OnecMonitorConnection connection, List<FileDto> files, CancellationToken cancellationToken)
    {
        ...
        foreach (var request in files.ToList().Select(fileToDownload => new FileRequestDto { Id = fileToDownload.Id }))
    ...
    private async void ConnectionOnMessageReceived(object? sender, Message e)
    {
        try
        {
            if (e.Header.Type == MessageType.V8FileChunk)
            {
                lock (_locker)
                {
                    var chunk = MessagePackSerializer.Deserialize<FileChunkDto>(e.Data);
