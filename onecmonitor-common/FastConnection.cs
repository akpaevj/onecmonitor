...
namespace OnecMonitor.Common
{
    public abstract class FastConnection : IDisposable
    {
        ...
        {
            var msg = await _inputChannel.Reader.ReadAsync(cancellationToken);
            RaiseIfError(msg, cancellationToken);
            
            return msg;
        }

        protected async Task WriteMessage(MessageType messageType, CancellationToken cancellationToken)
        {
            var header = new MessageHeader(messageType, 0, Guid.Empty);
            var message = new Message(header);

            await _outputChannel.Writer.WriteAsync(message, cancellationToken);
        }
        
        protected async Task WriteMessage<T>(MessageType messageType, T? item, Message? callMessage, CancellationToken cancellationToken)
        {
        ...
        protected async Task<TResult> WriteMessageAndWaitResult<TResult>(
        ...
        protected async Task<TResult> WriteMessageAndWaitResult<T, TResult>(
            MessageType messageType, 
            MessageType responseMessageType, 
            T item, 
            CancellationToken cancellationToken)
        ...
        private async Task<TResult> WriteMessageAndWaitResult<TResult>(Message message, MessageType responseMessageType, CancellationToken cancellationToken)
        {
            ...
            RaiseIfError(result, cancellationToken);
        ...
        private static void RaiseIfError(Message message, CancellationToken cancellationToken)
        {
